...
// defines local tracks to be published
var localTracks = {
...
$(() => {
    // options are taken from the query parameters
...
$("#join-form").submit(async function(e) {
    ...
    try {
        ...
        if (options.token) {
            // if the project is using tokens
...
async function join() {
    ...
    [options.uid, localTracks.audioTrack, screenTrack] = await Promise.all([
        ...
        }), AgoraRTC.createScreenVideoTrack({ // Check the README.md file for more details
            encoderConfig: "720p"
        }, "auto")]);
    ...
    // play local video track, similar to the videoSDK play() method.
...
async function leave() {
    for (trackName in localTracks) {
        ...
        if (track) {
            // stops all the local tracks and clears them
            track.stop();
            track.close();
    ...
    // leave the channel
    // Syncing the DOM with the channel status
    await client.leave();
...
async function subscribe(user, mediaType) {
    const uid = user.uid; // the UID of the remote user
