...
async function initDevices() {
    if (!localTracks.audioTrack) {
        // creates an audio track, this object can be used to change the state of the local audio track
        localTracks.audioTrack = await AgoraRTC.createMicrophoneAudioTrack({
            encoderConfig: "music_standard"
        });
    }
    if (!localTracks.videoTrack) {
        // creates a vide track, this object can be used to change the state of the local video track
        localTracks.videoTrack = await AgoraRTC.createCameraVideoTrack({
            encoderConfig: curVideoProfile.value
        });
    }

    // get mics. See the README.md file for more information on the getMicrophones() method.
    mics = await AgoraRTC.getMicrophones();

    // See the README.md file for more information on the getTrackLabel() method.
    const audioTrackLabel = localTracks.audioTrack.getTrackLabel();
    currentMic = mics.find(item => item.label === audioTrackLabel);
    $(".mic-input").val(currentMic.label);
    $(".mic-list").empty();
    // Creates a dropdown list of all possible mics to be chosed from
    mics.forEach(mic => {
        $(".mic-list").append(`<a class="dropdown-item" href="#">${mic.label}</a>`);
    });

    // get cameras. See the README.md file for more information on the getCameras() method.
    ...
    $(".cam-list").empty();
    // Creates a dropdown list of all possible cameras to be chosed from
    cams.forEach(cam => {
...
async function changeVideoProfile(label) {
    ...
    // See the README.md file for more information on the setEncoderConfiguration() method.
...
$(() => {
    ...
    // Gets the rooom info form the url query parameters
...
$("#join-form").submit(async function(e) {
    ...
    try {
        ...
        if (options.token) {
            // This only displays when the application is using a token
...
async function join() {
    ...
    // See the README.md file for more information on the play() method.
...
async function leave() {
    for (trackName in localTracks) {
        ...
        if (track) {
            // Stops the local track and cleans it up.
            track.stop();
            track.close();
            localTracks[trackName] = undefined; // cleanup
        }
    }
    ...
    // Sync the DOM with the channel status
...
async function subscribe(user, mediaType) {
    ...
    if (mediaType === "video") {
        // creates a container element for remote users' videos
        const player = $(`
...
function handleUserPublished(user, mediaType) {
    const id = user.uid;
    // Adds the new user to the remoteUsers object.
    remoteUsers[id] = user;
    // Subscribes to the new users' tracks.
...
/**
 * Gets the specified codec from the DOM.
 */
function getCodec() {
